...
4. Prompt, cancellation, approval, questions, session operations, and model/preset/permission actions. The first increment adds the wire and client method layer: `CoreClient` can answer core-initiated requests, `src/experimentalCoreClient.ts` provides typed TUI-client methods, and `src/core-bridge.ts` exposes `session/new`, `session/resume`, `session/list`, `session/rewind`, `model/list`, `model/switch`, `preset/list`, `preset/switch`, `permission/list`, and `permission/switch` while forwarding approval and user-question requests to the client. These methods are wired into the experimental TUI screen; real-TTY interactive acceptance is complete.
