...
| Launcher/profile/runtime install | `cute-dsh-tui.js`, `launch-options.js`, `src/profileManager.ts`, `src/update.ts` | `verify-launch-options.mjs`, `verify-profile-manifest-recovery.mjs`, `verify-profile-development-link.mjs`, `verify-profile-native-build.mjs`, `verify-package-exports.mjs` |
| V2 core/TUI transport | `src/core-protocol.ts`, `src/core-client.ts`, `src/core-bridge.ts`, `src/experimentalCoreClient.ts`, `src/sessionEventProjection.ts`, `src/experimentalNotificationBuffer.ts`, `src/experimentalProjection.tsx`, `core-bridge.patch.yml`, `docs/architecture/v2-runtime-boundary.md` | `verify-core-protocol.ts`, `verify-core-client.ts`, `verify-core-client-request-handler.mjs`, `verify-core-bridge.ts`, `verify-core-bridge-interactive.mjs`, `verify-v2-client-methods.mjs`, `verify-core-bridge-dsh.ts`, `verify-session-event-projection.ts`, `verify-session-event-client.ts`, `verify-session-open-buffer.ts`; default v1 launcher stays unchanged; `--experimental-v2` is the explicit experimental path |
