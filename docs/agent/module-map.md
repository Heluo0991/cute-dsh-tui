...
| V2 core/TUI transport | `src/core-protocol.ts`, `src/core-bridge.ts`, `core-bridge.patch.yml`, `docs/architecture/v2-runtime-boundary.md` | `verify-core-protocol.ts`, `verify-core-bridge.ts`; v1 launcher must remain unmodified until an experimental path is verified |
