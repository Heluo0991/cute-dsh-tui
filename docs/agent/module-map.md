...
| V2 core/TUI transport | `src/core-protocol.ts`, `src/core-client.ts`, `src/core-bridge.ts`, `src/sessionEventProjection.ts`, `src/experimentalProjection.tsx`, `core-bridge.patch.yml`, `docs/architecture/v2-runtime-boundary.md` | `verify-core-protocol.ts`, `verify-core-client.ts`, `verify-core-bridge.ts`, `verify-core-bridge-dsh.ts`, `verify-session-event-projection.ts`, `verify-session-event-client.ts`; default v1 launcher stays unchanged; `--experimental-v2` is the explicit read-only path |
