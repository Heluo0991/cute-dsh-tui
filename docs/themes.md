...
CuteDshTui 提供三套 Gentle Mist Blue 色板：
...
```text
CUTE_DSH_TUI_THEME
  > ~/.cute-dsh-tui/theme.json 中的持久化选择
  > OSC 11 背景检测
  > dark 回退
```
...
选择器确认后立即热切换，并把选择写入 `~/.cute-dsh-tui/theme.json`。如果设置了
`CUTE_DSH_TUI_THEME`，它在下一次启动时仍然优先。

## 自定义主题

在 `~/.cute-dsh-tui/themes/` 下放置 JSON 文件。每个文件定义一个主题，并从一个内置
色板开始覆盖：
...
`~/.cute-dsh-tui/themes/`。修改这部分实现时必须保留路径约束。
