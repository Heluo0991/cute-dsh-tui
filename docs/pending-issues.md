...
> 历史交接快照：本文件保留旧问题背景、验证经验和当时的 Git 状态；其中
> commit、版本和远端状态不再是当前真相。新 agent 先读根 `AGENTS.md`，运行
> `pnpm agent:status`，再读 `docs/agent/current.md`。新的当前工作写入后者。

> 以下是旧交接时的项目链路、已完成事项、待办和验证方式。它仍可用于追溯
> 背景，但不应覆盖新的渐进阅读协议。
