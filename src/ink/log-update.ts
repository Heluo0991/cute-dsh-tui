...
export class LogUpdate {
  ...
  render(
    ...
    if (
      ...
      // Inline output already owns terminal scrollback. Clearing and printing
      // the whole frame on resize leaves a second copy of the header/messages
      // behind in Windows ConPTY, so repaint only the reachable viewport.
      if (!altScreen) return repaintViewportInPlace(prev, next, stylePool)
    ...
    if (shrinking) {
      ...
      if (!altScreen && prev.screen.height > prev.viewport.height) {
        logForDebugging(
          `Viewport repaint (shrink tall): prevHeight=${prev.screen.height}, nextHeight=${next.screen.height}, viewport=${prev.viewport.height}, cursorAtBottom=${cursorAtBottom}`,
        )
      ...
      if (linesToClear > prev.viewport.height) {
        if (!altScreen) return repaintViewportInPlace(prev, next, this.options.stylePool)
