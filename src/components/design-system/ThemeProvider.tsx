 ...
 * `theme` prop, no CUTE_DSH_TUI_THEME override and no persisted choice
 * (~/.cute-dsh-tui/theme.json), it queries the terminal's background color
 ...
 * CUTE_DSH_TUI_THEME (built-in or user theme name) > persisted `/theme` choice >
 ...
 * validates the name, persists the choice to ~/.cute-dsh-tui/theme.json and hot
 * swaps the palette (and the module-level mirror) immediately.
 */

// User themes (~/.cute-dsh-tui/themes/<name>.json) resolve through this registry,
...
type ThemeContextValue = {
  ...
  theme: string
  /**
   * Switch themes at runtime. Persists to ~/.cute-dsh-tui/theme.json and hot
...
/**
 * CUTE_DSH_TUI_THEME skips terminal detection (tests, debugging). Accepts a
...
function envThemeOverride(): string | undefined {
  const v = process.env.CUTE_DSH_TUI_THEME
...
export function ThemeProvider({
  ...
  const [forcedValid] = useState<boolean>(() => {
    ...
    console.warn(
      `[cute-dsh-tui] theme "${forced}" not found (built-ins: light, dark, dark-ansi; user themes: ~/.cute-dsh-tui/themes/*.json); falling back to auto-detection`,
    )
    return false
  })
  ...
  const setTheme = React.useCallback((name: string): boolean => {
    if (!isThemeAvailable(name)) {
      console.warn(`[cute-dsh-tui] theme "${name}" not found`)
      return false
    }
    if (!writeThemePref(name)) {
      console.warn('[cute-dsh-tui] failed to write ~/.cute-dsh-tui/theme.json')
      return false
    }
    setActive(name)
    return true
