...
export function isMaxModel(model: { id: string; name: string }): boolean {
...
function mix(
  from: Readonly<{ r: number; g: number; b: number }>,
  to: Readonly<{ r: number; g: number; b: number }>,
  amount: number,
...
function rotatingColor(phase: number): `rgb(${number},${number},${number})` {
  const normalized = ((phase % 1) + 1) % 1
  const colors = [BLUE, SKY, ICE, WHITE] as const
 ...
 */
export function modelSwitchBorderColors(elapsedMs: number): ModelSwitchBorderColors {
...
export function modelSwitchGlowColor(elapsedMs: number): `rgb(${number},${number},${number})` {
