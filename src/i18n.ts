...
const dict = {
  ...
  'btw-done-hint': { zh: 'BTW 完成 · Ctrl+B 查看', en: 'BTW done · Ctrl+B to view' },
  'btw-view-hint': { zh: 'BTW 视图 · Ctrl+B 切换主会话', en: 'BTW view · Ctrl+B to switch to main' },
  ...
  'webui-url': { zh: '地址      {{url}}', en: 'URL       {{url}}' },
  'webui-launch': { zh: '启动      终端运行 dsh web（或 dsh --profile web；使用同一个 DSH_HOME）', en: 'Launch      run dsh web in a terminal (or dsh --profile web; same DSH_HOME)' },
  'webui-port': { zh: '端口      dsh web --port <端口>（默认 3080；0 由系统分配）', en: 'Port        dsh web --port <port> (default 3080; 0 lets the OS choose)' },
  'webui-host': { zh: '地址      仅本机 127.0.0.1；Web profile 尚未允许 --host 0.0.0.0', en: 'Host        loopback 127.0.0.1 only; the web profile rejects --host 0.0.0.0' },
  'webui-local-only': { zh: '说明      此命令只显示管理入口，不会写入会话日志或发送给模型', en: 'Note        this command only shows the management entry; it writes nothing to the session log and sends nothing to the model' },
  ...
  'btw-pane-hint': { zh: '已复制上下文 · 主会话不受影响 · Ctrl+B 切换 · Esc/Ctrl+C 停止 · Ctrl+O 详情', en: 'Forked context · main unchanged · Ctrl+B switch · Esc/Ctrl+C stop · Ctrl+O details' },
  ...
  'help-shortcut-btw': { zh: 'ctrl+b 切换 BTW', en: 'ctrl+b to switch BTW' },
  ...
  'status-hint-interrupt-btw': { zh: 'esc 中断 · ctrl+b BTW', en: 'esc to interrupt · ctrl+b BTW' },
