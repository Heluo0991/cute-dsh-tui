...
export function migrateLegacyPreferences({
  legacyDir = LEGACY_PREFERENCES_DIR,
  preferencesDir = PREFERENCES_DIR,
}: {
  legacyDir?: string
  preferencesDir?: string
} = {}): boolean {
  if (existsSync(preferencesDir) || !existsSync(legacyDir)) return false
  ...
  try {
    ...
    if (existsSync(legacyThemes) && lstatSync(legacyThemes).isDirectory()) {
      ...
      for (const entry of readdirSync(legacyThemes, { withFileTypes: true })) {
        if (entry.isFile()) copyFileSync(join(legacyThemes, entry.name), join(themesDir, entry.name))
      }
    }
    return true
  } catch {
