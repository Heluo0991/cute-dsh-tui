 ...
 */
function readWindowsClipboard(): Promise<ClipboardContent> {
...
async function readLinuxClipboard(): Promise<ClipboardContent> {
  ...
  for (const command of ['xclip', 'xsel']) {
    if (!existsSync(`/usr/bin/${command}`) && !existsSync(`/usr/local/bin/${command}`)) continue
 ...
 */
export function readClipboard(): Promise<ClipboardContent> {
  if (process.platform === 'win32') return readWindowsClipboard()
  if (process.platform === 'darwin') return readTextClipboard('pbpaste')
