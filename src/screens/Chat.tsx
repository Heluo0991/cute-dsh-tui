...
import type { Channel, ChatRow, PresetOption, ReasoningEffortOption } from '../channel.js'
...
import { parsePluginAction, pluginArgs, profileDependencies, type PluginAction } from '../pluginManager.js'
...
export function Chat({
  ...
  const [selectedModel, setSelectedModel] = React.useState<LlmModelInfo | null>(null)
  const [modelEfforts, setModelEfforts] = React.useState<readonly ReasoningEffortOption[]>([])
  ...
  const [resumeExpandedGroups, setResumeExpandedGroups] = React.useState<ReadonlySet<string>>(() => new Set())
  ...
  const resumeVisibleSessions = React.useMemo(
    () => resumeSessions.filter(session => session.isLeaf || resumeExpandedGroups.has(session.lineageRoot)),
    [resumeSessions, resumeExpandedGroups],
  )
  ...
  const cyclePermission = (): void => {
    ...
    if (next !== undefined) requestPermission(next.id)
  }

  /** Complete the second `/model` stage after its reasoning depth is known. */
  const switchModelWithEffort = (model: LlmModelInfo, effort?: ReasoningEffortOption): void => {
    ...
    if (showSwitchGlow) setMaxModelSwitchesInFlight(count => count + 1)
    channel.notify(`Switching model to ${model.name}${effort ? ` · ${effort.name}` : ''}…`)
    void channel
      .switchModel(model.provider, model.id, effort?.id)
      .then(ok => {
        if (ok) channel.notify(`Model switched to ${model.name}${effort ? ` · ${effort.name}` : ''}`)
      })
      .catch(() => {
    ...
    <Box flexDirection="column" flexGrow={1} width="100%">
      ...
      <ScrollBox ref={setHandle} flexDirection="column" flexGrow={1} flexShrink={1} stickyScroll>
        <LogoHeader
          ...
          cwd={channel.cwd}
          animateIntro={fullscreen || channel.rows.length === 0}
        />
      ...
      <Box flexDirection="column" flexShrink={0}>
        ...
        {resumePickerOpen && resumeSessions.length > 0 && (
          <Box flexDirection="column" marginTop={1}>
            <ResumePicker
              sessions={resumeVisibleSessions}
        ...
        {effortPickerOpen && selectedModel !== null && (
          <Box flexDirection="column" marginTop={1}>
        ...
        {questionSnapshot === null && approvalSnapshot === null && (
          <PromptInput
            channel={channel}
            modelSwitching={maxModelSwitchesInFlight > 0}
            helpOpen={helpOpen}
