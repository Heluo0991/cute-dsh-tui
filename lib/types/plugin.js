...
import { pluginArgs } from './pluginManager.js';
import { createSessionCredentialLease } from './sessionCredential.js';
...
export async function apply(ctx, config) {
    ...
    // pretending the update landed.
    {
        ...
        if (updatedFrom !== undefined) {
            ...
            if (now === undefined || !isVersionNewer(now, updatedFrom)) {
                ctx.logger.warn(`cute-dsh-tui: /update restarted but the version did not advance (still ${now ?? 'unknown'}, was ${updatedFrom})`);
    ...
    // DSH's approval service is channel-neutral.  This TUI is the terminal
    // answerer for the agent it currently owns; requests from subagents or
    // another frontend delegate to the next answerer instead of stealing their
    // prompt.  The store settles every request once and cancels cleanly during
    // teardown, so `ask` remains fail-closed if the UI disappears.
    const approvalStore = new ApprovalStore();
    ctx.on('approval/request', (request, next) => {
        if (request.agent.id !== channel.agentId)
            return next();
        return approvalStore.ask(request);
    });
    ctx.effect(() => () => approvalStore.cancelAll());
    ...
    const sessionCredential = createSessionCredentialLease(() => ctx.get('credentials'));
    const applySessionCredential = async (key) => {
        try {
    ...
    const releaseSessionCredential = async () => {
        try {
    ...
    const funnel = createExitFunnel({
        onUserExit: async (error) => {
            ...
            if (pluginAction !== undefined) {
                ...
                disposeRootAndThen(ctx, () => {
                    void runProfilePluginAndRestart(channel.agentId, profile, pluginArgs(action)).then(({ pluginCode, restartCode }) => {
                        if (pluginCode !== 0) {
    ...
    const chat = React.createElement(Chat, {
        ...
        // Exact resume injects sessionId before boot; bare `--resume` opens the
        ...
        yoloResumeUpgrade: process.env.CUTE_DSH_TUI_YOLO === '1' &&
        ...
        onPluginAction: profile === undefined ? undefined : action => {
            if (exited || updateRequested || pluginAction !== undefined)
                return;
            pluginAction = action;
            instance?.unmount();
        },
...
async function resolveAgent(ctx, requestedSessionId, configuredRoute, startupRoute, meta, configuredPreset) {
    ...
    if (requestedSessionId !== undefined) {
        ...
        try {
            ...
            ctx.logger.warn(`cute-dsh-tui: resume of "${requestedSessionId}" failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    if (rejected !== undefined) {
        ctx.logger.warn(`cute-dsh-tui: model route ${rejected.provider}/${rejected.model} is not advertised by provider "${rejected.provider}"; falling back to ${route.provider}/${route.model}`);
    }
    const created = await ctx.agents.create({
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`cute-dsh-tui: failed to create agent (provider=${route.provider}, model=${route.model}): ${message}`);
    });
