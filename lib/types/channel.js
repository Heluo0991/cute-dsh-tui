 ...
 */
export function bashHeredocWriteDiff(command) {
...
function foldRows(rows, cap) {
    ...
    for (const row of rows.slice(0, excess)) {
        if (row.folded || row.restored)
            continue;
        if (row.kind !== 'user' && row.kind !== 'context' && row.kind !== 'assistant' && row.kind !== 'reasoning' && row.kind !== 'tool')
            continue;
        row.folded = true;
        folded += 1;
...
export function createChannel(ctx, initialAgent, options) {
    ...
    const permissionStateFor = (target) => {
    ...
    const checkContextWarning = () => {
        ...
        state.notify(t('context-low', { percent: percentLeft }), { color: 'warning', timeoutMs: 8000 });
    };
    /**
    ...
    const updateBtwThread = (id, patch) => {
    ...
    const mainProgress = () => {
        const tail = agent.session.events.slice(-24).map(event => {
            if (event.type === 'tool/call')
                return `tool started: ${event.data.name} ${event.data.arguments.slice(0, 600)}`;
            if (event.type === 'tool/result')
                return 'tool settled';
            if (event.type === 'assistant/chunk') {
            ...
            if (event.type === 'user/message')
                return 'main user message is in progress';
            return event.type;
        });
        return [
    ...
    state = {
        ...
        async startBtw(question) {
                    ...
                    setup: async (agentCtx) => {
                        ...
                        agentCtx.tools.register(defineTool({
                            ...
                            output: {
                                ...
                                render: (_args, value) => [{ type: 'text', text: value.progress }],
                            },
                            execute: async () => ({
            ...
            const textOfChild = (content) => (content ?? []).map(block => block.type === 'text' ? block.text : '').join('').trim();
            const firstTextOfChild = (content) => (content ?? []).find(block => block.type === 'text')?.text.trim() ?? '';
            const dispose = ctx.on('session/event', (session, event) => {
                ...
                if (event.type === 'user/message') {
                    if (event.data.source.kind === 'user') {
                        ...
                        if (text)
                            thread.rows.push({ id: nextId++, kind: 'user', text });
                    }
                }
        ...
        cancelBtw(id) {
            ...
            const thread = state.btwThreads.find(item => item.id === id);
            if (thread !== undefined) {
                updateBtwThread(id, { working: false });
        ...
        submitBtw(id, text) {
            ...
            const entry = btwHandles.get(id);
            const thread = state.btwThreads.find(item => item.id === id);
            if (entry === undefined || thread === undefined || prompt === '')
                return;
        ...
        async switchPermission(presetId) {
            ...
            try {
                const execution = await commandService.execute(agent, `/permission ${presetId}`, [], new AbortController().signal);
                ...
                state.notify(t('permission-switch-failed', { error: error instanceof Error ? (error.message ?? String(error)) : String(error) }), { color: 'error', timeoutMs: 8000 });
                return false;
            }
        },
        listModels() {
        ...
        async listSessions() {
            ...
            try {
                ...
                // Match Claude Code's workspace scoping: the picker lists sessions
                // created from this exact working directory only.
                const cwd = state.cwd.replace(/\/+$/, '');
                const scoped = headers.filter(header => (header.cwd ?? '').replace(/\/+$/, '') === cwd
        ...
        compact() {
            ...
            void compactService
                ...
                .catch((error) => {
                state.notify(t('compaction-failed', { error: error instanceof Error ? error.message : String(error) }), { color: 'error', timeoutMs: 8000 });
            });
        },
