 ...
 */
export function Chat({ channel, questionStore, approvalStore: suppliedApprovalStore, onExit, onUpdate, onPluginAction, onStoreCredential, onForgetCredential, profile, fullscreen = false, openResumePickerOnStart, yoloResumeUpgrade, }) {
        ...
        switch (name) {
            ...
            case 'btw': {
                ...
                if (question === '') {
                    ...
                    if (latest === undefined)
                        channel.notify('No BTW conversation yet. Use /btw <question>.');
                    else {
                        setBtwDraft('');
            ...
            if (key.return) {
                ...
                void channel.switchPermission('danger-full-access').then((ok) => {
                    if (ok && fromYoloResume)
                        channel.notify('Yolo upgrade enabled for this resumed session');
                });
            }
            else if (key.escape) {
                        ...
                        (channel.activityEnabled &&
                            ...
                            void (async () => {
                                ...
                                channel.notify(ok
                                    ...
                                    : 'Could not save or apply the API key. Check the terminal error message and try again.', { color: ok ? 'success' : 'error' });
                            })();
                        }, onDecline: () => {
                            setPendingLoginKey(null);
                            ...
                            void (async () => {
                                ...
                                channel.notify(ok ? 'Saved CuteDshTui credential removed.' : 'Could not remove the saved credential.', { color: ok ? 'success' : 'error' });
                            })();
                        }, onCancel: () => {
                            setCredentialDeleteConfirm(false);
