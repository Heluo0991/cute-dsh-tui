...
import { BtwPane } from '../components/BtwPane.js';
import { parsePluginAction, pluginArgs, profileDependencies } from '../pluginManager.js';
 ...
 * tools (the corresponding SKILL.md ships under ~/.dsh/skills with cute-dsh-tui).
 */
 ...
 */
export function Chat({ channel, questionStore, approvalStore: suppliedApprovalStore, onExit, onUpdate, onPluginAction, onStoreCredential, onForgetCredential, onApplySessionCredential, onReleaseSessionCredential, profile, fullscreen = false, openResumePickerOnStart, yoloResumeUpgrade, }) {
    ...
    const approvalStore = React.useMemo(() => suppliedApprovalStore ?? new ApprovalStore(), [suppliedApprovalStore]);
    ...
    const [modelEfforts, setModelEfforts] = React.useState([]);
    const [effortIndex, setEffortIndex] = React.useState(0);
    /** Count avoids an overlapping route request clearing a still-live glow. */
    const [maxModelSwitchesInFlight, setMaxModelSwitchesInFlight] = React.useState(0);
    ...
    const [resumeExpandedGroups, setResumeExpandedGroups] = React.useState(() => new Set());
    ...
    /** `/theme` color-theme picker (built-ins + ~/.cute-dsh-tui/themes user themes). */
    ...
    const [permissionConfirm, setPermissionConfirm] = React.useState(null);
    ...
    const [btwOpenId, setBtwOpenId] = React.useState(null);
    const [btwPendingId, setBtwPendingId] = React.useState(null);
    const [btwDraft, setBtwDraft] = React.useState('');
    const [pluginConfirm, setPluginConfirm] = React.useState(null);
    ...
    const [pendingLoginKey, setPendingLoginKey] = React.useState(null);
    ...
    // title when set, else "CuteDshTui"; a `⠂/⠐` spinner prefix while a turn is
    // working (960ms cadence, only while the terminal is focused), a static
    // `✦` otherwise. CuteDshTui brands the idle prefix with the DeepSeek whale.
    ...
    const resumeVisibleSessions = React.useMemo(() => resumeSessions.filter(session => session.isLeaf || resumeExpandedGroups.has(session.lineageRoot)), [resumeSessions, resumeExpandedGroups]);
    const openStartupResumePicker = React.useCallback(() => {
        void (async () => {
            ...
            setResumeExpandedGroups(new Set());
    ...
    React.useEffect(() => {
        if (!openResumePickerOnStart || startupResumeOpenedRef.current)
            return;
    ...
    const yoloResumePromptedSessionsRef = React.useRef(new Set());
    React.useEffect(() => {
        if (!yoloResumeUpgrade)
            return;
        const sessionId = channel.agentId;
        if (yoloResumePromptedSessionsRef.current.has(sessionId))
            return;
    ...
    const requestPermission = (presetId, fromYoloResume = false) => {
    ...
    /** Shift+Tab advances the same safe permission flow as `/permission`. */
    const cyclePermission = () => {
        ...
        const next = permissions.options[(current + 1 + permissions.options.length) % permissions.options.length];
        if (next !== undefined)
            requestPermission(next.id);
    };
    /** Complete the second `/model` stage after its reasoning depth is known. */
    const switchModelWithEffort = (model, effort) => {
        const showSwitchGlow = isMaxModel(effort ?? model);
        // A local session fork can settle within one React event batch. Keeping a
        // short minimum interval ensures Max's transition is actually painted at
        // least once instead of opening and closing before the terminal redraws.
        const glowStartedAt = showSwitchGlow ? Date.now() : 0;
        if (showSwitchGlow)
            setMaxModelSwitchesInFlight(count => count + 1);
        channel.notify(`Switching model to ${model.name}${effort ? ` · ${effort.name}` : ''}…`);
        void channel
            .switchModel(model.provider, model.id, effort?.id)
            .then(ok => {
            if (ok)
                channel.notify(`Model switched to ${model.name}${effort ? ` · ${effort.name}` : ''}`);
        })
            .catch(() => {
            // Channel normally turns failures into notifications. This guard still
            // guarantees visual cleanup for a rejected host promise.
        })
            .finally(() => {
            if (!showSwitchGlow)
                return;
            const remaining = Math.max(0, 1600 - (Date.now() - glowStartedAt));
            setTimeout(() => {
                setMaxModelSwitchesInFlight(count => Math.max(0, count - 1));
            }, remaining);
        });
    };
    /**
    ...
    React.useEffect(() => {
        if (btwPendingId === null || channel.working)
            return;
        if (!channel.btwThreads.some(thread => thread.id === btwPendingId))
            return;
    ...
    const runCommand = (name, rawInput = '') => {
        switch (name) {
            case 'activity': {
                ...
                // ~/.cute-dsh-tui/working-activity.json and survives restarts.
            ...
            case 'preset': {
                ...
                // default for future sessions (~/.cute-dsh-tui/agent-preset.json).
                const parts = rawInput.trim().split(/\s+/).filter(Boolean);
                if (parts[0] === 'status') {
                    setHelpOpen(false);
            ...
            case 'lang': {
                ...
                // (hot-swap, persisted to ~/.cute-dsh-tui/lang.json). Precedence on next
                // launch: CUTE_DSH_TUI_LANG > cordis.yml `lang` > the persisted choice.
                const parts = rawInput.trim().split(/\s+/).filter(Boolean);
                if (parts[0] === 'status') {
                    setHelpOpen(false);
            ...
            case 'theme': {
                ...
                // palettes + user themes from ~/.cute-dsh-tui/themes); `/theme <name>`
                // switches directly; `/theme status` shows the current choice.
                // Selection persists to ~/.cute-dsh-tui/theme.json and hot swaps via the
                // ThemeProvider setter (CUTE_DSH_TUI_THEME still wins on next launch).
                const parts = rawInput.trim().split(/\s+/).filter(Boolean);
                if (parts[0] === 'status') {
                    setHelpOpen(false);
            ...
            case 'btw': {
                ...
                if (question === '') {
                    ...
                    if (latest === undefined)
                        channel.notify(t('btw-none-yet'));
                    else
                        openBtwWhenSettled(latest.id);
                    return true;
                }
                void channel.startBtw(question).then(id => {
                    if (id !== undefined)
                        openBtwWhenSettled(id);
                });
                return true;
            }
            case 'plugin': {
                const input = rawInput.trim();
                if (input === '' || input === 'list' || input.startsWith('search ')) {
                    const filter = input.startsWith('search ') ? input.slice('search '.length).toLowerCase() : '';
                    const installed = profileDependencies(profile);
                    const loaded = channel.listLoadedPlugins();
                    const lines = [
                        ...installed.map(name => `installed  ${name}`),
            ...
            case 'config': {
                ...
                const lines = [
                    t('doctor-example-config', { path: 'dsh --profile cute-dsh-tui' }),
                    t('doctor-user-config', { path: `${userHome}/.dsh/profiles/cute-dsh-tui/cordis.patch.yml` }),
                    '',
            ...
            case 'logout':
                ...
                if (onReleaseSessionCredential !== undefined) {
                    void onReleaseSessionCredential().then(released => {
                        if (!released)
                            channel.notify(t('login-release-failed'), { color: 'warning' });
                    });
                }
            ...
            case 'permissions':
                ...
                channel.pushLocal('/permissions', [
                    ...
                    ...channel.permissions.options.map(option => `${option.id === channel.permissions?.current ? '✓ ' : '  '}${option.name} — ${option.description}`),
    ...
    useInput((input, key, event) => {
        ...
        if (loginOpen || pendingLoginKey !== null || credentialDeleteConfirm)
            return;
        if (approvalSnapshot !== null) {
            if (key.return)
                approvalStore.allowCurrent();
            else if (key.escape || (key.ctrl && input === 'c'))
                approvalStore.cancelCurrent();
            else if (input === 'd' || input === 'D')
                approvalStore.rejectCurrent();
        ...
        if (btwOpenId !== null) {
            if (key.escape)
                ...
                if (btwDraft.trim()) {
                    channel.submitBtw(btwOpenId, btwDraft);
                    setBtwDraft('');
                }
            }
            else if (key.backspace) {
                setBtwDraft(value => value.slice(0, -1));
            }
            else if (!key.ctrl && !key.meta && input) {
                setBtwDraft(value => value + input);
            }
            event.stopImmediatePropagation();
            return;
        }
        if (pluginConfirm !== null) {
            if (key.return) {
                const action = pluginConfirm;
        ...
        if (permissionConfirm !== null) {
            if (key.return) {
                ...
                void channel.switchPermission('danger-full-access').then((ok) => {
                    if (ok && fromYoloResume)
                        channel.notify(t('yolo-enabled-resumed'));
                });
            }
            else if (key.escape) {
        ...
        if (resumePickerOpen) {
            if (key.upArrow) {
                setResumeIndex(index => (index <= 0 ? resumeVisibleSessions.length - 1 : index - 1));
            }
            else if (key.downArrow) {
                setResumeIndex(index => (index >= resumeVisibleSessions.length - 1 ? 0 : index + 1));
            }
            else if (key.rightArrow) {
                const session = resumeVisibleSessions[resumeIndex];
                if (session !== undefined) {
                    setResumeExpandedGroups(previous => new Set(previous).add(session.lineageRoot));
                }
            }
            else if (key.leftArrow) {
                const session = resumeVisibleSessions[resumeIndex];
                if (session !== undefined) {
                    setResumeExpandedGroups(previous => {
                        const next = new Set(previous);
                        next.delete(session.lineageRoot);
                        return next;
                    });
                    setResumeIndex(0);
                }
                ...
                if (session) {
                    ...
                    void channel.resumeTo(session.id).then((ok) => {
                        if (ok)
                            channel.notify(t('session-resumed'));
                    });
                }
                else {
                    setResumePickerOpen(false);
        ...
        if (effortPickerOpen) {
            if (key.upArrow) {
                setEffortIndex(index => (index <= 0 ? modelEfforts.length - 1 : index - 1));
            }
            else if (key.downArrow) {
                setEffortIndex(index => (index >= modelEfforts.length - 1 ? 0 : index + 1));
            }
            else if (key.return) {
                const effort = modelEfforts[effortIndex];
                if (selectedModel !== null && effort !== undefined) {
                    setEffortPickerOpen(false);
                    switchModelWithEffort(selectedModel, effort);
                }
                else {
                    setEffortPickerOpen(false);
                }
        ...
        if (modelPickerOpen) {
            if (key.upArrow) {
                ...
                // oxlint-disable-next-line typescript/no-unnecessary-condition -- runtime guard: out-of-range index on an empty list
                if (model) {
                    // The first `/model` stage chooses a route. The second asks the
                    // adapter for that route's actual reasoning levels.
                    setSelectedModel(model);
                    void channel.listModelEfforts(model.provider, model.id).then(efforts => {
                        if (efforts.length === 0) {
                            setModelPickerOpen(false);
                            channel.notify(`${model.name} does not expose selectable reasoning depths; using its default`);
                            switchModelWithEffort(model);
                            return;
                        }
                        setModelEfforts(efforts);
                        const current = model.provider === channel.provider && model.id === channel.model
                            ? efforts.findIndex(effort => effort.id === channel.reasoningEffort)
                            : -1;
    ...
    const promptSelectionActive = selectionActive || modelPickerOpen || effortPickerOpen || resumePickerOpen || activityPickerOpen ||
    ...
    const btwUnseen = channel.btwThreads.reduce((count, thread) => (thread.working || thread.id === btwOpenId ? count : count + 1), 0);
    return (_jsxs(Box, { flexDirection: "column", flexGrow: 1, width: "100%", children: [_jsxs(Box, { display: btwViewActive ? 'none' : 'flex', flexDirection: "column", flexGrow: 1, children: [!isSticky && channel.lastUserText && (_jsx(StickyPromptHeader, { text: channel.lastUserText, onClick: () => {
                                ...
                                (channel.activityEnabled &&
                                    ...
                                    if (loginNeedsSaveConfirm) {
                                        ...
                                        void onApplySessionCredential(key).then(ok => {
                                            channel.notify(ok ? t('login-session-applied') : t('login-session-failed'), { color: ok ? 'success' : 'error' });
                                        });
                                    }
                                }, onCancel: () => setLoginOpen(false) })), pendingLoginKey !== null && (_jsx(CredentialSaveConfirm, { onConfirm: () => {
                                    ...
                                    void (async () => {
                                        ...
                                        channel.notify(ok
                                            ...
                                            : t('credential-save-failed'), { color: ok ? 'success' : 'error' });
                                    })();
                                }, onDecline: () => {
                                    const key = pendingLoginKey;
                                    setPendingLoginKey(null);
                                    ...
                                    void onApplySessionCredential(key).then(ok => {
                                        channel.notify(ok ? t('login-session-applied-exit') : t('login-session-failed'), { color: ok ? 'success' : 'error' });
                                    });
                                } })), credentialDeleteConfirm && (_jsx(CredentialDeleteConfirm, { onConfirm: () => {
                                    ...
                                    void (async () => {
                                        ...
                                        channel.notify(ok ? t('credential-removed') : t('credential-remove-failed'), { color: ok ? 'success' : 'error' });
                                    })();
                                }, onCancel: () => {
                                    setCredentialDeleteConfirm(false);
                                    channel.notify(t('logout-saved-kept'));
                                } })), permissionPickerOpen && channel.permissions !== undefined && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(PermissionPicker, { options: channel.permissions.options, focusIndex: permissionIndex, currentPreset: channel.permissions.current }) })), resumePickerOpen && resumeSessions.length > 0 && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(ResumePicker, { sessions: resumeVisibleSessions, focusIndex: resumeIndex, currentSessionId: channel.agentId }) })), modelPickerOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: models.length === 0 ? (_jsx(ModelPickerLoading, {})) : (_jsx(ModelPicker, { models: models, focusIndex: modelIndex, currentModel: `${channel.provider}/${channel.model}` })) })), effortPickerOpen && selectedModel !== null && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(ReasoningEffortPicker, { model: selectedModel, efforts: modelEfforts, focusIndex: effortIndex, currentEffort: channel.reasoningEffort }) })), activityPickerOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(ActivityPicker, { focusIndex: activityIndex, currentPreset: channel.activityFrames }) })), presetPickerOpen && presetOptions.length > 0 && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(PresetPicker, { presets: presetOptions, focusIndex: presetIndex, currentPreset: channel.agentPreset }) })), themePickerOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(ThemePicker, { focusIndex: themeIndex, currentTheme: themeName }) })), historyOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(HistorySearchDialog, { query: historyQuery, cursorOffset: historyCursor, matches: historyMatches, focusIndex: historyFocus }) })), rewindOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(RewindPicker, { rows: rewindRows, focusIndex: rewindIndex, confirmRow: rewindConfirm }) })), searchOpen && _jsx(TranscriptSearchBar, { query: searchQuery, cursorOffset: searchCursor, count: searchCount, current: searchCurrent }), _jsx(GoalTodoPanel, { channel: channel }), approvalSnapshot !== null && _jsx(ApprovalPanel, { approval: approvalSnapshot }), questionSnapshot !== null && (_jsx(AskUserQuestionPanel, { question: questionSnapshot.question, position: questionSnapshot.position, total: questionSnapshot.total, answered: questionSnapshot.answered, onAnswer: selection => questionStore.answerCurrent(selection), onCancel: () => questionStore.cancelCurrent() }, questionSnapshot.key)), questionSnapshot === null && approvalSnapshot === null && (_jsx(PromptInput, { channel: channel, modelSwitching: maxModelSwitchesInFlight > 0, helpOpen: helpOpen, onToggleHelp: () => { setHelpOpen(previous => !previous); }, onRunCommand: runCommand, selectionActive: promptSelectionActive, fillText: historyFill, onFillConsumed: () => { setHistoryFill(null); }, onRewindRequest: openRewind, onCyclePermission: cyclePermission, controllerRef: promptControllerRef, active: !btwViewActive }))] })] }), channel.btwThreads.length > 0 && (_jsx(Box, { display: btwViewActive ? 'flex' : 'none', flexDirection: "column", flexGrow: 1, children: btwThread !== undefined && (_jsx(BtwPane, { thread: btwThread, draft: btwDraft, expanded: expanded })) })), _jsx(StatusLine, { channel: channel, selectionActive: selectionActive, helpOpen: helpOpen, btwUnseen: btwUnseen, btwViewActive: btwViewActive })] }));
}
/**
