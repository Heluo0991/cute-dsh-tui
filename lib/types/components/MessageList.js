...
export function MessageList({ rows, expanded, expandedRows, selectedId, onToggleRow, model, showAll, onToggleAll, onLoadOlder, thinkingVisible = true, registerRowRef, scrollHandle, forceMountRowId, newSinceRowId, onUnseenCount, }) {
    ...
    return (_jsxs(_Fragment, { children: [rows.some(row => row.folded) && (_jsx(Box, { marginTop: 1, onClick: onLoadOlder, children: _jsx(Divider, { title: t('load-earlier') }) })), !showAll && hiddenCount > 0 && (_jsx(Box, { marginTop: 1, onClick: onToggleAll, children: _jsx(Divider, { title: t('show-previous-messages', { count: hiddenCount }) }) })), topPad > 0 && _jsx(Box, { height: topPad, flexShrink: 0 }), visibleRows
                .slice(start, end)
                .map((row) => {
                ...
                const tool = row.tool;
                return (_jsx(MemoRow, { rowId: row.id, kind: row.kind, label: row.label, text: row.text, streaming: row.streaming === true, durationMs: row.durationMs, time: row.time, addMargin: addMargin, isSelected: selectedId === row.id, isExpanded: expandedRows.has(row.id), expanded: expanded, model: model, background: rowBackground(row.id), toolCallId: tool?.callId, toolName: tool?.name, toolArgsText: tool?.argsText, toolArgsFull: tool?.argsFull, toolStatus: tool?.status, toolResultText: tool?.resultText, toolResultFull: tool?.resultFull, toolErrorText: tool?.errorText, toolCallView: tool?.callView, toolResultView: tool?.resultView, toolStartedAt: tool?.startedAt, toolDurationMs: tool?.durationMs, nowSec: tool?.status === 'running' ? nowSec : undefined, onToggleRow: onToggleRow, setRowRef: setRowRef }, row.id));
            }), bottomPad > 0 && _jsx(Box, { height: bottomPad, flexShrink: 0 })] }));
}
function TranscriptRow({ rowId, kind, label, text, streaming, durationMs, time, addMargin, isSelected, isExpanded, expanded, model, background, toolCallId, toolName, toolArgsText, toolArgsFull, toolStatus, toolResultText, toolResultFull, toolErrorText, toolCallView, toolResultView, toolStartedAt, toolDurationMs, onToggleRow, setRowRef, }) {
    ...
    switch (kind) {
        ...
        case 'context':
            return (_jsx(Box, { marginTop: addMargin ? 1 : 0, paddingLeft: 2, backgroundColor: background, ref: ref, onClick: onClick, children: expanded || isExpanded ? (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Text, { color: "permission", bold: true, children: ["\u2301 ", label ?? t('injected-context-label')] }), _jsx(Text, { dimColor: true, wrap: "wrap", children: text })] })) : (_jsxs(Text, { dimColor: true, italic: true, children: ["\u2301 ", label ?? t('injected-context-label'), " \u2014 ", compactPreview(text), ' ', _jsx(KeyboardShortcutHint, { shortcut: "ctrl+o", action: t('action-expand'), parens: true })] })) }));
        case 'assistant':
        ...
        case 'reasoning':
            return (_jsx(Box, { flexDirection: "column", ref: ref, children: _jsx(AssistantThinkingMessage, { thinking: text, addMargin: addMargin, 
                    ...
                    verbose: isExpanded || expanded, durationMs: durationMs, streaming: streaming, isSelected: isSelected, onClick: onClick }) }));
        case 'tool': {
        ...
        case 'compact':
            ...
            // the full summary.
            return (_jsx(Box, { marginTop: addMargin ? 1 : 0, paddingLeft: 2, backgroundColor: background, ref: ref, onClick: onClick, children: expanded || isExpanded ? (_jsx(Text, { dimColor: true, children: text })) : (_jsxs(Text, { dimColor: true, italic: true, children: ["\u2234 ", t('compact-summary-folded'), " \u00B7 ", compactPreview(text), ' ', _jsx(KeyboardShortcutHint, { shortcut: "ctrl+o", action: t('action-expand'), parens: true })] })) }));
    }
}
 ...
 */
export function LogoHeader({ model, effort, cwd, animateIntro = true, }) {
    return (_jsx(Box, { flexDirection: "column", marginBottom: 1, children: _jsx(LogoV2, { model: model, effort: effort, cwd: cwd, animateIntro: animateIntro }) }));
}
