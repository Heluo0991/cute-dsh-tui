...
function HighlightedCode({ code, language, dimColor = false, }) {
    ...
    return _jsx(Text, { dimColor: dimColor, children: rendered });
}
...
function CommandLines({ command, language, verbose, columns, }) {
    ...
    return (_jsxs(_Fragment, { children: [code !== '' && (_jsxs(Box, { flexDirection: "row", width: "100%", children: [_jsx(Box, { width: 5, flexShrink: 0, children: _jsx(Text, { dimColor: true, children: GUTTER_FIRST }) }), _jsx(Box, { flexGrow: 1, children: _jsx(HighlightedCode, { code: code, language: language }) })] })), hints.map((line, index) => (_jsxs(Box, { flexDirection: "row", width: "100%", children: [_jsx(Box, { width: 5, flexShrink: 0, children: _jsx(Text, { dimColor: true, children: GUTTER_REST }) }), _jsx(Box, { flexGrow: 1, children: _jsx(Text, { dimColor: true, wrap: "wrap", children: line.text }) })] }, `hint-${index}`)))] }));
}
 ...
 *  its original shape. */
function contentTextLines(text) {
    if (text === '')
        return [];
...
function diffLines(diffs) {
    ...
    for (const diff of diffs) {
        ...
        if (newLines.length > 1)
            out.push(add(`+${newLines.length} lines`));
        if (oldLines.length > 1)
            out.push(del(`-${oldLines.length} lines`));
...
function viewLines(view) {
    switch (view.card) {
        ...
        case 'terminal': {
            ...
            const lines = contentTextLines(output).map(dim);
...
function visualLineCount(lines, width) {
    return lines.reduce((total, line) => total + Math.max(1, Math.ceil(stringWidth(line.text) / width)), 0);
}
/**
 ...
 */
function capLines(lines, max, verbose, columns) {
    ...
    if (visualLineCount(lines, bodyWidth) <= max)
        return lines;
...
export function AssistantToolUseMessage({ tool, addMargin, verbose, isSelected = false, isExpanded = false, }) {
    ...
    // the command area.
    const command = headerIsTerminal && tool.callView?.card === 'terminal' ? tool.callView.title : '';
    ...
    return (_jsx(Box, { ref: viewportRef, flexDirection: "row", justifyContent: "space-between", marginTop: addMargin ? 1 : 0, width: "100%", backgroundColor: isSelected
            ...
            : isExpanded
                ...
                : undefined, children: _jsxs(Box, { flexDirection: "column", flexGrow: 1, children: [_jsxs(Box, { flexDirection: "row", flexWrap: "nowrap", minWidth: minWidth, children: [_jsx(ToolUseLoader, { shouldAnimate: isRunning, isUnresolved: isRunning, isError: isError }), headerIsTerminal ? (_jsx(Box, { flexShrink: 0, children: _jsx(Text, { bold: true, wrap: "truncate-end", children: name }) })) : (_jsx(HeaderTitle, { name: name, title: headerTitle, isTerminal: false, displayArgs: displayArgs })), !isRunning && (_jsx(Box, { flexWrap: "nowrap", children: _jsx(Text, { dimColor: true, children: elapsedText }) }))] }), command !== '' && (_jsx(CommandLines, { command: command, language: commandLanguage(tool.name), verbose: verbose, columns: columns })), lines.map((line, index) => (_jsxs(Box, { flexDirection: "row", width: "100%", backgroundColor: line.tone === 'add'
                        ? 'diffAdded'
                        : line.tone === 'del'
                            ? 'diffRemoved'
                            : undefined, children: [_jsx(Box, { width: 5, flexShrink: 0, children: _jsx(Text, { dimColor: true, children: index === 0 ? GUTTER_FIRST : GUTTER_REST }) }), _jsx(Box, { flexGrow: 1, children: _jsx(Text, { color: line.tone === 'add'
                                    ? 'diffAddedWord'
                                    : line.tone === 'del'
                                        ? 'diffRemovedWord'
                                        : line.tone === 'error'
