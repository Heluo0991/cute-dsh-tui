...
export function renderCuteDeepSeekRows(pulse = 0) {
    ...
    for (let y = 0; y < SPRITE.length; y += 2) {
        ...
        for (let x = 0; x < upper.length; x++) {
            ...
            if (up !== undefined && down !== undefined) {
                style = fg(brighten(up, amount)) + bg(brighten(down, amount));
                glyph = '▀';
            }
            else if (up !== undefined) {
                style = fg(brighten(up, amount));
                glyph = '▀';
            }
            else if (down !== undefined) {
                style = fg(brighten(down, amount));
                glyph = '▄';
            }
            if (style !== current) {
                out += style === '' ? RESET : style;
                current = style;
            }
            out += glyph;
        }
...
export function CuteDeepSeekArt({ pulse, width }) {
