 ...
 * order) followed by discovered user themes from ~/.cute-dsh-tui/themes (sorted
 ...
 * ~/.cute-dsh-tui/themes — each row shows the display name, its base and three
 * key color swatches; `❯` marks focus, `✓` the active theme. Enter applies
 * through the ThemeProvider setter (persists to ~/.cute-dsh-tui/theme.json and
 * hot swaps), Esc cancels.
 */
export function ThemePicker({ focusIndex, currentTheme, }) {
    const options = React.useMemo(() => getThemeOptions(), []);
    return (_jsx(Pane, { color: "permission", children: _jsxs(Box, { flexDirection: "column", children: [_jsx(Box, { marginBottom: 1, children: _jsx(Text, { color: "remember", bold: true, children: "Color theme" }) }), _jsx(Select, { options: options, focusIndex: focusIndex, selectedValue: currentTheme, visibleOptionCount: 6 }), _jsx(Text, { dimColor: true, italic: true, children: _jsxs(Byline, { children: [_jsx(KeyboardShortcutHint, { shortcut: "Enter", action: t('action-confirm'), bold: true }), _jsx(KeyboardShortcutHint, { shortcut: "Esc", action: t('action-exit') })] }) })] }) }));
}
