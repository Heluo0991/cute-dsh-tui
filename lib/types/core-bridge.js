...
export async function apply(ctx, config) {
    ...
    const replaceActive = async (next) => {
    ...
    ctx.on('approval/request', (request, next) => {
        ...
        return transport.request('approval/request', params).then(outcome => {
    ...
    transport.onRequest(async (request) => {
        ...
        if (request.method === 'session/rewind') {
            ...
            if (session.agent.status === 'running')
                throw new Error('session/rewind is unavailable while the agent is running');
            const seq = requiredNumber(params, 'seq');
            const sessions = ctx.get('sessions');
            if (sessions === undefined)
                throw new Error('session/rewind is unavailable: no session store');
        ...
        if (request.method === 'model/switch') {
            ...
            requireSession(params, session.agent.session.id);
            if (session.agent.status === 'running')
                throw new Error('model/switch is unavailable while the agent is running');
            const provider = requiredString(params, 'provider');
            const model = requiredString(params, 'model');
            const sessions = ctx.get('sessions');
            if (sessions === undefined)
                throw new Error('model/switch is unavailable: no session store');
        ...
        if (request.method === 'preset/switch') {
            ...
            try {
                ...
                const appendable = session.agent.session;
...
async function openForkedAgent(ctx, options) {
    const agentOptions = {
        ...options.provider === undefined ? {} : { provider: options.provider },
        ...options.model === undefined ? {} : { model: options.model },
    };
...
async function listSessions(ctx, cwd) {
...
async function listModels(ctx) {
    ...
    try {
        ...
        return providers.flatMap((provider, index) => lists[index]?.map(model => ({ provider: provider.id, model: model.id })) ?? []);
    }
    catch {
        return [];
    }
...
function rewindBoundary(events, rowSeq) {
    let boundary = rowSeq;
