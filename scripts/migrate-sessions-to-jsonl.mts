 ...
 * store (`~/.cute-dsh-tui/sessions.sqlite`) into the shared JSONL store
 ...
 *
 * Defaults: --from $CUTE_DSH_TUI_SESSION_ROOT ?? ~/.cute-dsh-tui/sessions.sqlite
...
const from = argValue('--from') ?? process.env.CUTE_DSH_TUI_SESSION_ROOT ?? join(homedir(), '.cute-dsh-tui', 'sessions.sqlite')
