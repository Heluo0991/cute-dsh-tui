...
process.env.CUTE_DSH_TUI_THEME = 'dark'     // 跳过 OSC 11 探测，保持确定性
...
// 完整 buffer 落盘（CUTE_DSH_TUI_REPRO_DUMP 指定路径时），供离线分析残留分布。
if (process.env.CUTE_DSH_TUI_REPRO_DUMP) {
  const fs = await import('node:fs')
  fs.writeFileSync(process.env.CUTE_DSH_TUI_REPRO_DUMP, lines.map((l, i) => `${String(i).padStart(4)}|${l}`).join('\n'))
  console.log(`buffer 已落盘: ${process.env.CUTE_DSH_TUI_REPRO_DUMP}`)
  ...
  fs.writeFileSync(process.env.CUTE_DSH_TUI_REPRO_DUMP + '.frames', dump2)
  console.log(`稳态帧序列已落盘: ${process.env.CUTE_DSH_TUI_REPRO_DUMP}.frames`)
}
